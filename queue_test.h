...
template<>
void producer<moodycamel::ReaderWriterQueue<int>, int>(moodycamel::ReaderWriterQueue<int> &queue, const int &d, boost::atomic_int &counter, unsigned iterations, bool use_counter)
{
   
    for (int i = 0; i != iterations; ++i) {
        if(queue.enqueue( d) && use_counter) {
           ++counter;
        }
    }
...
template<>
void producer<moodycamel::ReaderWriterQueue<std::string>, std::string>(moodycamel::ReaderWriterQueue<std::string> &queue, const std::string &data, boost::atomic_int &counter, unsigned iterations, bool use_counter)
{
...
template<>
void consumer<moodycamel::ReaderWriterQueue<std::string>, std::string > (moodycamel::ReaderWriterQueue<std::string> &queue,  boost::atomic<bool> &done, boost::atomic_int &counter, bool use_counter)
{
...
template<>
void consumer<moodycamel::ReaderWriterQueue<int>, int > (moodycamel::ReaderWriterQueue<int> &queue,  boost::atomic<bool> &done, boost::atomic_int &counter, bool use_counter)
{
...
void run_test() {
    ...
    {
            boost::lockfree::spsc_queue<int> queue(init_size);
            run_queue_benchmark<boost::lockfree::spsc_queue<int>, int >("boost::lockfree::spsc_queue", queue, 1, 10000000, true, 1,1);       
    }
    
    {
            moodycamel::ReaderWriterQueue<int> queue(init_size);
            run_queue_benchmark<moodycamel::ReaderWriterQueue<int>, int >("moodycamel::ReaderWriterQueue", queue, 1, 10000000, true, 1,1);       
    }
    
    {
        for(unsigned i = 0; i < 4; i++) {
            moodycamel::ConcurrentQueue<int> queue(init_size);
...
void test_1KQueueSize() {
    std::cout << "Testing 100 bytes with initial queue size 1024*1024" << std::endl;
     unsigned init_size = 1024*1024;
    ...
    
    {
        ...
        {
            moodycamel::ReaderWriterQueue<std::string> queue(init_size);
            run_queue_benchmark<moodycamel::ReaderWriterQueue<std::string>, std::string >("moodycamel::ReaderWriterQueue", queue, data,  10000000, true, 1,1);       
        }
    }
}
void test_1MB_QueueSize() {
    std::cout << "Testing 100 bytes with initial queue size 1024*1024*1024" << std::endl;
...
void test_Default_QueueSize() {
    std::cout << "Testing 100 bytes with default queue size" << std::endl;
